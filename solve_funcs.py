...
def valid(
    ...
    ]  # Create a list of the numbers in the column
    if (
        number in column
...
def create_priority(
    ...
    """Creates an nested list of containing the number of valid values for each location and another nested list containing empty values replaced with an array of all valid values.
    ...
    Returns:
        priority_array (tuple[list[list[list[int] | int]]): Nested list containing the coordinates of each point in the Sudoku grid and potential values - sorted against the number of potential values
        valid_array (list[list[list[int] | int]]]): A nested list containing the grid with zeroes replaced with all valid values for that location in the Sudoku grid
    """
    priority_array = []
                ...
                for value in valid_array_old[row][column]:
                    if valid(
                        ...
                        valid_array[row][column].append(
                            value
                        )  # Append valid value to corresponding list in valid_array
                        priority_array[-1][2].append(
                            value
    ...
    if (
        ...
        priority_array[0][2] = [
            priority_array[0][2]
        ]  # Create a list of length 1 containing the single valid value
...
def recursive_solve(
    ...
    if priority_array:  # Check if priority_array is empty
        ...
        grid[row][
            column
        ] = 0  # Reset grid to original state if all values have been attempted and none are valid
...
def wavefront_solve(
    ...
    priority_array_update = copy.deepcopy(
        priority_array
    )  # Create a copy of priority_array
    valid_array_update = copy.deepcopy(
        valid_array
    )  # Create a copy of valid_array
    while priority_array_update:  # Repeat until priority_array_update is empty
        if (
            ...
            if valid(
                ...
                priority_array_update = priority_array_update[
                    1:
                ]  # Remove first element in priority_array_update
            else:
                return False, False
        else:
            ...
            if valid(
                ...
                grid_update_2 = copy.deepcopy(
                    grid_update
                )  # Create a copy of the updated grid
                grid_update_2[priority_array_update[0][0]][
                    priority_array_update[0][1]
                ] = test_num  # Update location in grid with randomly chosen number from priority_array
                (
                ...
                if not priority_array_update_2[0][2]:
                    priority_array_update[0][2].remove(
                        test_num
                ...
                if (
                    not grid_update_2
                ):  # Check if number was invalid from previous recursion level
                    priority_array_update[0][2].remove(
                        test_num
                    )  # Remove invalid value from current recursion level
            else:
                priority_array_update[0][2].remove(
                    test_num
                )  # Remove invalid value from current recursion level
            if (
                "grid_update_2" in locals() and grid_update_2
            ):  # If grid_update_2 was created for this recursion level and is not False
